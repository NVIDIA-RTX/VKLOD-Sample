...
public:
  ...
  {
    auto lhs = reinterpret_cast<uintptr_t>(m_ptr);
    auto rhs = reinterpret_cast<uintptr_t>(other.m_ptr);
...
private:
  ...
  static pointer offsetBytes(pointer ptr, const difference_type& byteOffset)
  {
...
public:
  ...
  DynamicArrayView(std::vector<std::remove_cv_t<ValueType>>& vector)
      ...
      , m_resizeCallback([this, &vector](size_type size, const ValueType& value) {
        vector.resize(size, value);
        this->m_ptr  = vector.data();
        this->m_size = size;
        return this->m_ptr;
      })
  {
